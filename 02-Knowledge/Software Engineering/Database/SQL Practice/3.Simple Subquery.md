...
```sql
SELECT GV.HOTEN, COUNT(DT.MADT) AS SLDTCM
FROM GIAOVIEN GV
	JOIN DETAI DT ON GV.MAGV = DT.GVCNDT
GROUP BY GV.MAGV, GV.HOTEN
ORDER BY SLDTCM DESC
LIMIT 1 OFFSET 0
    
```

- Dùng COUNT và MAX (xử lý được cả trường hợp trùng nhiều người có chung số lượng đề tại chủ nhiệm là max)
```sql
SELECT GV.HOTEN, COUNT(DT.MADT) AS SLDTCM
FROM GIAOVIEN GV
	JOIN DETAI DT ON GV.MAGV = DT.GVCNDT
...
HAVING COUNT(DT.MADT) = (
	SELECT MAX(T.SLDTCM)
    FROM (
    	SELECT COUNT(DT1.MADT) AS SLDTCM
		FROM GIAOVIEN GV1 
